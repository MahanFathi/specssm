from typing import Protocol, Callable, Optional, TypeVar, Dict, Tuple, Union
import jax
...
def create_lra_preprocess(input_len: int, in_dim: int) -> Callable:
	...
	def prep_batch(batch: tuple) -> Tuple[jnp.ndarray, jnp.ndarray, jnp.array]:
		"""
		...
		return full_inputs, targets.astype(float)

	return prep_batch


# Example interface for making a loader.
class CustomLoaderFn(Protocol):
    def __call__(cache_dir: str,
				  bsz: int = 50,
