...
class StackedEncoderModel(nn.Module):
    """ Defines a stack of SSM layers to be used as an encoder.
        Args:
            ...
            seq_layer   (nn.Module): the sequence layer to be used
            num_layers  (int32):    the number of S5 layers to stack
    """
    input_len: int
    d_model: int = gin.REQUIRED
    training: bool = True
    seq_layer: nn.Module = gin.REQUIRED
    ...
    def setup(self):
        ...
        self.layers = [
            self.seq_layer(input_len=self.input_len, training=self.training) 
...
#     variable_axes={
#         "params": None, "dropout": None, 'batch_stats': None, "cache": 0, "prime": None}, 
#     split_rngs={
...
class ClassificationModel(nn.Module):
    """ Classificaton sequence model. This consists of the stacked encoder
    ...
    def setup(self):
        ...
        """
        self.encoder = StackedEncoderModel(input_len=self.input_len, d_model=self.d_model, training=self.training)
